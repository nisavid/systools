...
- Use the ubiquitous language in `tools/mlxctl/CONTEXT.md`: Server Definition, Server Type, Model Alias, Model Reference, Client Endpoint, Upstream Endpoint, Supervisor, Probe, Request Metric Event, Process Sample, and Metric Query.
- Keep package code under `tools/mlxctl/src/mlxctl` and mirror behavior in focused `tools/mlxctl/tests/test_*.py` modules.
