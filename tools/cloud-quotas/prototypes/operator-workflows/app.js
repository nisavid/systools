...
function renderEvidenceFirst() {
  return `${heading("Variant B · quota inspector", "Inspect quota and Spot advice", "Browse accelerator constraint sets, select one exact slice, and compare supported capacity advice without confusing likelihood with quota or guaranteed capacity.")}
  <div class="ledger-layout">
    ...
    <section class="ledger-workspace">
      <div class="ledger-toolbar"><button class="button">Filter: H100</button><button class="button">Region: all</button><button class="button">State: all</button><button class="button">Capacity: Spot advice</button><span class="grow"></span><span class="state mutable"><span class="signal signal-good">●</span>live provider evidence</span></div>
      ...
      <div class="inspector"><section><div class="eyebrow">Selected exact slice</div><h2>H100 family · us-central1</h2><dl class="kv-grid"><dt>Canonical identity</dt><dd>atlas-ml-dev / compute.googleapis.com / GPUS-PER-GPU-FAMILY / region=us-central1,gpu_family=H100</dd><dt>Applicable locations</dt><dd>us-central1</dd><dt>Constraint set</dt><dd>regional 8 · all-regions 12 ◆ bottleneck at workload 16</dd><dt>Evidence</dt><dd>QuotaInfo 42s · usage metric 61s · complete</dd></dl></section><section class="capacity-advice"><div class="eyebrow">Spot capacity advice · Preview</div><div class="advice-score"><strong>0.82</strong><span>HIGH OBTAINABILITY</span></div><dl class="kv-grid"><dt>Configuration</dt><dd>a3-highgpu-8g × 2 · Spot</dd><dt>Distribution</dt><dd>ANY_SINGLE_ZONE</dd><dt>Estimated uptime</dt><dd>≥ 60 minutes</dd><dt>30-day preemption</dt><dd>11–27% by zone</dd></dl><p class="subtle">Synthetic sample · provider advice 48s ago. Likelihood, not a capacity guarantee.</p></section><section><div class="eyebrow">Valid next operations</div><div class="actions"><button class="button">Resolve requirement</button><button class="button">Compare Spot zones</button><button class="button primary" data-action="preview">Preview amendment</button><button class="button">Watch settled preference</button></div><p class="subtle">Mutation acts on this one selected slice. Capacity advice is read-only and configuration-specific.</p></section></div>
      ${planDrawer()}
    </section>
  </div>`;
}
